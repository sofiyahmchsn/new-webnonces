<div class="site-wrap">
      <div class="site-section block-3 site-blocks-2 bg-light">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-md-7 site-section-heading text-center pt-4">
              <h2>Featured Products</h2>
            </div>
          </div>
          <div class="row">
            <div class="col-md-12">
              <div class="nonloop-block-3 owl-carousel">
                <div class="item">
                  <div class="block-4 text-center">
                    <figure class="block-4-image">
                      <img
                        src="images/baked-potatoe.jpg"
                        alt="Image placeholder"
                        class="img_menu"
                      />
                    </figure>
                    <div class="block-4-text p-4">
                      <h3><a href="#">Baked Potatoe</a></h3>
                      <p class="mb-0">20x20</p>
                      <p class="text-primary font-weight-bold">Rp.175.000</p>
                    </div>
                  </div>
                </div>
                <div class="item">
                  <div class="block-4 text-center">
                    <figure class="block-4-image">
                      <img
                        src="images/cake-coklat-mini.jpg"
                        alt="Image placeholder"
                        class="img_menu"
                      />
                    </figure>
                    <div class="block-4-text p-4">
                      <h3><a href="#">Cake Coklat</a></h3>
                      <p class="mb-0">8x8</p>
                      <p class="text-primary font-weight-bold">Rp.100.000</p>
                    </div>
                  </div>
                </div>
                <div class="item">
                  <div class="block-4 text-center">
                    <figure class="block-4-image">
                      <img
                        src="images/cinnamon-rolls.jpg"
                        alt="Image placeholder"
                        class="img_menu"
                      />
                    </figure>
                    <div class="block-4-text p-4">
                      <h3><a href="#">Cinamon Roll</a></h3>
                      <p class="mb-0">20x20</p>
                      <p class="text-primary font-weight-bold">Rp.225000</p>
                    </div>
                  </div>
                </div>
                <div class="item">
                  <div class="block-4 text-center">
                    <figure class="block-4-image">
                      <img
                        src="images/klappertart-box.jpg"
                        alt="Image placeholder"
                        class="img_menu"
                      />
                    </figure>
                    <div class="block-4-text p-4">
                      <h3><a href="#">Klappertart</a></h3>
                      <p class="mb-0">20x20</p>
                      <p class="text-primary font-weight-bold">Rp.175000</p>
                    </div>
                  </div>
                </div>
                <div class="item">
                  <div class="block-4 text-center">
                    <figure class="block-4-image">
                      <img
                        src="images/spagheti-brulee.jpg"
                        alt="Image placeholder"
                        class="img_menu"
                      />
                    </figure>
                    <div class="block-4-text p-4">
                      <h3><a href="#">Spaghetti Brulle</a></h3>
                      <p class="mb-0">20x20</p>
                      <p class="text-primary font-weight-bold">Rp.175000</p>
                    </div>
                  </div>
                </div>
              </div>
              <p class="text-center mt-5">
                <a href="index.php?page=menu" class="btn btn-primary btn-sm text-center">Shop Now</a>
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>

